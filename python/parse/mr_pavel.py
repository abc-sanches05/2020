...
def pgsql_insert(lstapart):
    ...
    cursor = conn.cursor()
    cursor.execute('INSERT INTO mr_pavel ( apart, price, price_m, date, hull, floor, size, finish) VALUES (%s, %s, %s, CURRENT_DATE,%s, %s, %s, %s)', (lstapart[0], lstapart[1], lstapart[2],lstapart[3], lstapart[4], lstapart[5],lstapart[6],))
    conn.commit()
    cursor.close()
    conn.close()
...
def pages(site):
    ...
    conn = psycopg2.connect(dbname='test', user='postgres', password='110167', host='localhost')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO count_apart ( desc_apart, developer, date, all_apart) VALUES (%s, %s, CURRENT_DATE, %s )', ('pavel', 'mr', all_apart,))
    conn.commit()
    cursor.close()
    conn.close()
...
def parse(site):
    ...
    result = re.findall(r'a href="/catalog/apartments/db.*catalog-item__col _favorite-wrap', r_e, flags=re.DOTALL)
    print(type(result))
    result2 = ''.join(result)
...
all_apart = pages('https://www.mr-group.ru/catalog/apartments/?project=41666&type=12&min_price=&max_price=')
# студии павелецкая
